...
#----------------------------------------------------------------#
# Section one: 
# This part of the code should the basic request for the accounts.py weekly tasks.
# The code has been left in the file to show how it would be done but coded out so that only the extra task runs for dynamic accounts.  
...
print(f'{no_of_Xs}{dynamic_last_4_nos}') # prints the end result for the user by printing the amount of X's calculated above directly followed by the last 4 digits